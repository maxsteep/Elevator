/*  Elevator control program
	global registers:
	r5  previous keyboard value is copied here if we arrive from the LEGO triggered safety interrupt
	r15 keyboard value
...
GPIO_1_ISR: #LEGO
	...
	mov r5, r15#save previous ps2 button value (Desired floor) to the global r5
...
PB_ONE:#KEY0
	...
	#br LEGO_ON_TIMER_ON - should never get triggered if math is right
...
SAFETY:
	cmpeqi r16, r12, 0x1B
	beq r16, r0, END_ISR#if neither allowable key was pressed, exit
	
	movi r15, 0b1000
	
	br bodyPS2
	
	#now r15 holds the pushbutton format value
bodyPS2:		
#pb paste start
	#unnecessary, but lets leave for now, is it really? check later<24>
	stw ra, 104(sp)
	...
	#if snapshot has value -> go to safety
	movia r2, 0x02FAF080#sec
	beq r23, r2, continuePS2
	movia r2, 0x05F5E100#2 sec
	beq r23, r2, continuePS2
	
	#beq r23, r17, continue#safety value check
	beq r23, r0, continuePS2#checking remaining timer value, probably unneeded, check - <
	beq r17, r0, continuePS2#if flag = 0 continue
	#there will be a case where timer has an unclean value but jump should happen via r17 = 0
	br safetyPS2
	#if 0, clean exit, do below
...
continuePS2:	
	...
	#r13 is temp, holding the pre-defined value for pre-defined floors
	movi r13,0b0001	#we need to something to compare against
	beq r15,r13, PS2_ONE	#comparison and jump
	
	movi r13,0b0010
	beq r15,r13, PS2_TWO
	
	movi r13,0b0100
	beq r15,r13, PS2_THREE
	
safetyPS2:#remaining timer snapshot value triggered
	movi r13,0b1000
	beq r15,r13, PS2_FOUR
